...
from flash_whisper import get_transcription, delete_old_tmp_folders

import os
import subprocess

# 通过 subprocess 运行 Python 命令来获取库路径
...
def generate_file(file_obj):
    try:
        ...
        context = get_transcription(file_obj.name)
        # 获取拷贝在临时目录的新的文件地址
        print(context)
...
def main():
    ...
    with tempfile.TemporaryDirectory(dir='.') as tmpdir:
        ...
        # 创建 Gradio 应用程序g
        app = gr.Interface(fn=generate_file, inputs=inputs, outputs=outputs,   title="会议纪要生成器 meeting whisper flash",
                      description="生成英文纪要，等音频文件上传完成再submit，模型加载、处理时间较长，请耐心等待，不要断网，不要刷新界面 不要切屏待机 处理时间参考:45分钟录音->处理时间5分钟"
        )

        # 启动应用程序
        app.launch(server_name='0.0.0.0', server_port=7857)
